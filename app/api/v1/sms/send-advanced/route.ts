...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isAdmin) {
      ...
      if (userCepsmsUsername && userCepsmsUsername.trim() !== '') {
        ...
        if (!account) {
          ...
          console.warn('[SMS Send Advanced] Kullanıcı hesabı bulunamadı, varsayılan hesaba fallback yapılıyor:', {
            userId: auth.user.id,
        ...
        console.warn('[SMS Send Advanced] Kullanıcıya CepSMS hesabı atanmamış, varsayılan hesap kullanılıyor:', {
          userId: auth.user.id,
        });
        userCepsmsUsername = undefined; // undefined = varsayılan hesap kullan
      }
      ...
      console.log('[SMS Send Advanced] Admin kullanıcı için Smsexp hesabı kullanılıyor:', {
        userId: auth.user.id,
    ...
    console.log('[SMS Send Advanced] SMS gönderim öncesi:', {
      userId: auth.user.id,
    ...
    // İlk numaraya SMS gönder (kullanıcıya özel hesap ile)
    ...
    if (smsResult.success && smsResult.messageId) {
      ...
      if (!isAdmin && userCredit !== undefined) {
        ...
        if (updateError) {
          console.error('[SMS Send Advanced] Kredi düşürme hatası:', {
            userId: auth.user.id,
            phone: firstPhone,
            requiredCredit,
            userCredit,
      ...
      if (createError || !smsMessageData) {
        ...
        if (!isAdmin && userCredit !== undefined) {
          ...
          if (refundError) {
            console.error('[SMS Send Advanced] Kredi geri verme hatası:', {
              userId: auth.user.id,
              phone: firstPhone,
