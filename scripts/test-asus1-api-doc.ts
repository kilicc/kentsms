/**
 * asus1 hesabı - CepSMS API V1.1.3 dokümantasyonuna göre test
 * 
 * Kullanım:
 *   npx tsx scripts/test-asus1-api-doc.ts
 */
...
async function testAsus1WithAPIDoc() {
  console.log('=== asus1 Hesabı - CepSMS API V1.1.3 Format Testi ===\n');

  // asus1 hesabını al
  const account = getAccountByUsername('asus1');
  
  if (!account) {
    console.error('❌ asus1 hesabı bulunamadı!');
    process.exit(1);
  }
  ...
  // 3. Send SMS (Gelişmiş Format - Turkish Coding ile)
  console.log('\n' + '='.repeat(80));
  console.log('3️⃣  Send SMS (Gelişmiş Format - Turkish Coding):');
  console.log('='.repeat(80));
  ...
  try {
    const turkishRequest = {
      ...
      User: account.username,
      Pass: account.password,
      Message: 'Merhaba test - Türkçe karakterler: ığüşöç',
      Coding: 'turkish',
      StartDate: null,
      ValidityPeriod: 1140,
      Numbers: [testPhone],
    ...
    console.log('\n📤 Request:');
    console.log(JSON.stringify({
      From: turkishRequest.From,
      User: turkishRequest.User,
      Pass: '***',
      Message: turkishRequest.Message,
      Coding: turkishRequest.Coding,
      StartDate: turkishRequest.StartDate,
      ValidityPeriod: turkishRequest.ValidityPeriod,
      Numbers: turkishRequest.Numbers,
    }, null, 2));

    const turkishResp = await axios.post(endpoint, turkishRequest, {
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
      },
    ...
    console.log(`\n📥 Status Code: ${turkishResp.status}`);
    console.log('📥 Response:');
    console.log(JSON.stringify(turkishResp.data, null, 2));
    
    const status = turkishResp.data?.Status || turkishResp.data?.status;
    if (status === 'OK' || status === 200) {
      console.log('\n✅ BAŞARILI! SMS gönderildi.');
      if (turkishResp.data?.MessageId) {
        console.log(`   MessageId: ${turkishResp.data.MessageId}`);
      }
    } else if (status === 'User Error') {
      console.log('\n❌ User Error - Kullanıcı adı veya şifre hatalı');
    } else if (status === 'System Error') {
...
// Script'i çalıştır
testAsus1WithAPIDoc().catch(error => {
