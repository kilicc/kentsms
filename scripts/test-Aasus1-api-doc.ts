/**
 * Aasus1 hesabı - CepSMS API V1.1.3 dokümantasyonuna göre test
 * 
 * Kullanım:
 *   npx tsx scripts/test-Aasus1-api-doc.ts
 */
...
async function testAasus1WithAPIDoc() {
  console.log('=== Aasus1 Hesabı - CepSMS API V1.1.3 Format Testi ===\n');

  // Aasus1 hesabını al
  const account = getAccountByUsername('Aasus1');
  
  if (!account) {
    console.error('❌ Aasus1 hesabı bulunamadı!');
    process.exit(1);
  }
...
// Script'i çalıştır
testAasus1WithAPIDoc().catch(error => {
