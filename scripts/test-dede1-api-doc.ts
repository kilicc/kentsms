/**
 * dede1 hesabı - CepSMS API V1.1.3 dokümantasyonuna göre test
 * 
 * Kullanım:
 *   npx tsx scripts/test-dede1-api-doc.ts
 */
...
async function testDede1WithAPIDoc() {
  console.log('=== dede1 Hesabı - CepSMS API V1.1.3 Format Testi ===\n');

  // dede1 hesabını al
  const account = getAccountByUsername('dede1');
  
  if (!account) {
    console.error('❌ dede1 hesabı bulunamadı!');
    process.exit(1);
  }
...
// Script'i çalıştır
testDede1WithAPIDoc().catch(error => {
