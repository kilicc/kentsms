/**
 * Varsayılan .env hesabı testi (Test / Tkaipd771!)
 * 
 * Kullanım:
 *   npx tsx scripts/test-default-env.ts
 */
...
async function testDefaultEnv() {
  console.log('=== Varsayılan .env Hesabı - CepSMS API V1.1.3 Format Testi ===\n');

  const username = process.env.CEPSMS_USERNAME;
  const password = process.env.CEPSMS_PASSWORD;

  if (!username || !password) {
    console.error('❌ CEPSMS_USERNAME veya CEPSMS_PASSWORD .env dosyasında bulunamadı!');
    process.exit(1);
  }

  console.log('✅ Hesap bilgileri (.env\'den):');
  ...
  try {
    const advancedRequest = {
      From: process.env.CEPSMS_FROM || 'CepSMS',
      User: username,
      Pass: password,
      Message: testMessage,
      Coding: 'default',
